    ...
    internal static class Program
    {
        ...
        private static void Clean(string fileName)
        {
            try
            {
                ...
                if (Directory.Exists(PathMgr.Lua(fileName).Replace("\\CAB-ios32", ""))) Utils.Rmdir(PathMgr.Lua(fileName).Replace("\\CAB-ios32", ""));

                foreach (var mod in ListOfMod)
                {
                    if (File.Exists(PathMgr.Temp(mod))) File.Delete(PathMgr.Temp(mod));
                    if (Directory.Exists(PathMgr.Lua(mod).Replace("\\CAB-ios32", ""))) Utils.Rmdir(PathMgr.Lua(mod).Replace("\\CAB-ios32", ""));
                }
            }
            catch (Exception e)
            {
